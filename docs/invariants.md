...
- **The curriculum rule, as the owner revised it.** The rule used to end
  ...
  - **Formations** — the shard shapes on the tier ladder — now complete by
    **dl 520**, level 5's finish line, NOT dl 340. That boundary moved when the
    ...
    tier to `LV[LV.length-2].end`. (The previous text of this entry said dl
    340, and was left half-rewritten when the boundary moved — the sentence
    that followed it had lost its subject. Formations are the kind that has to
    finish early: they compound with each other, they are what the death coach
...
- **THE HOUSE STYLE — three rules, and they are what "cohesive" meant.** The
  ...
  2. **One counter idiom.** `NOUN ×N` is a multiplier — `COMBO ×3`,
     ...
     clock — `SHIELDS · 3`, `CLEAN LAPS · 3`, `BLACK HOLE · 12s`,
