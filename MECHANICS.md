...
| "extended slow motion" | 0.55× — the same factor the slow-mo orb already uses, so the mode's biggest visual signature was one the player had seen. Particles integrated on raw `dt`, so debris kept moving at full speed through it. | 0.42× (0.26× on entry), and particles run on dilated time. |
