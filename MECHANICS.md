...
| Red in the sky | The ban on the red family is **lifted** (owner's call, against a stated risk to the "red means danger" contract). What replaces it is narrower: `SKY_ARENA_CALM` compresses contrast in the annulus the orbits occupy, so EMBERFALL can burn at the rim while the band a shard is read against stays quiet. Hue is free; contrast directly behind the rings is not. | (internal — set it to 0 for the unmoderated sky) |
