...
| System | How it works | Explained by |
|---|---|---|
| Worlds | Eight of them, in journey order — DRIFT, TIDE, DUSTLANE, GLASS, EMBERFALL, VEIL, GRID, DEEP FIELD — and they differ in **structure**, not just in colour. **The order is the level order.** `LEVEL_HOME` floors each level's opening sky, and because a floor is a `Math.max` against a journey that never runs backwards, a table that dips has dead entries: it read `[0,1,5,3,2,7]`, so levels 4 and 5 asked for worlds already behind them and no run from level 1 ever saw either home. `[0,1,2,3,4,7]` climbs — LIFT OFF/DRIFT, INTO THE RINGS/TIDE, THE STORM/DUSTLANE, EVENT HORIZON/GLASS, REDSHIFT/EMBERFALL, HEAT DEATH/DEEP FIELD — and VEIL and GRID are what you travel *through*, VEIL because it is GRID's bright neighbour and the brightness gate fails without it. A world is a set of weights over four structures the shader recovers from fields it already evaluates: soft billows, large-scale mass, contours (the level sets of those fields, which is what makes curtains free) and the ridged network as light. Each also carries its own dust depth, void, star gain, temperament (`motion` 0.60–1.30) and palette. The weights are interpolated on the CPU, so the shader never branches on a world and a transition is filaments dissolving into billows rather than a crossfade. | wordless — arriving somewhere is the explanation |
